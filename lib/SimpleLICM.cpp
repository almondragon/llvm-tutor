...
struct SimpleLICM : public PassInfoMixin<SimpleLICM> {
  PreservedAnalyses run(Loop &L, LoopAnalysisManager &AM,
    ...
    SmallVector<Instruction *, 32> Worklist; // worklist
    ...
    for (BasicBlock *BB: L.getBlocks()) {
      for (Instruction &I : *BB) {
        ...
        Worklist.push_back(&I); // adding candidate instructions
      }
    }
    ...
    while (Modified) {
      ...
      for (Instruction *I: Worklist) {
        ...
        for (Value *Op : I->operands()) {
          ...
          if (Instruction *InstWOp = dyn_cast<Instruction>(Op)){
            if (L.contains(InstWOp) && !InvariantSet.count(InstWOp)) {
              isInvariant = false;
              break;
            }
          }
